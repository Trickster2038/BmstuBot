...
async def select_option(message: types.Message):
    ...
    key = types.InlineKeyboardButton(text="Изменить профиль", callback_data= "menu_change")
    keyboard.add(key)
    key = types.InlineKeyboardButton(text="Изменить аватар", callback_data= "menu_avatar")
    keyboard.add(key)
    key = types.InlineKeyboardButton(text="Верифицировать аккаунт", callback_data= "menu_verify")
    keyboard.add(key)
    key = types.InlineKeyboardButton(text="Показать мой профиль", callback_data= "menu_show_profile")
    keyboard.add(key)
    key = types.InlineKeyboardButton(text="Удалить аккаунт", callback_data= "menu_delete")
    keyboard.add(key)
