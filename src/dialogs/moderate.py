...
async def moderate(message):
    ...
    if check_id != None:
        ...
        key = types.InlineKeyboardButton(text="Подтвердить", callback_data= "submit_" + str(check_id))
        keyboard.add(key)
        key = types.InlineKeyboardButton(text="Отклонить", callback_data= "discard_" + str(check_id))
        keyboard.add(key)
...
def register_handlers_moderate(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(callback_moderate, lambda call: call.data.startswith("submit_") or call.data.startswith("discard_"))