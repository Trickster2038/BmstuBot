...
async def process_surname_valid(message: types.Message):
    ...
    for i in range(len(faculties)):
        key = types.InlineKeyboardButton(
            text=faculties[i][0], callback_data=str(i))
        keyboard.add(key)
...
def register_handlers_register(dp: Dispatcher):
    ...
    dp.register_message_handler(process_name_invalid, lambda message: not dbutils.validShortString(
        message.text), state=RegisterStates.name)
    dp.register_message_handler(process_name_valid, lambda message: dbutils.validShortString(
        message.text), state=RegisterStates.name)
    dp.register_message_handler(process_surname_invalid, lambda message: not dbutils.validShortString(
        message.text), state=RegisterStates.surname)
    dp.register_message_handler(process_surname_valid, lambda message: dbutils.validShortString(
        message.text), state=RegisterStates.surname)
    dp.register_callback_query_handler(
        callback_faculty, state=RegisterStates.faculty)
    dp.register_callback_query_handler(
        callback_department, state=RegisterStates.department)
    dp.register_callback_query_handler(
        callback_course, state=RegisterStates.course)
    dp.register_message_handler(process_bio_invalid, lambda message: not dbutils.validLongString(
        message.text), state=RegisterStates.bio)
    dp.register_message_handler(process_bio_valid, lambda message: dbutils.validLongString(
        message.text), state=RegisterStates.bio)
