...
async def process_avatar(message: types.Message, state: FSMContext):
    ...
    path = "../web/proj/media/avatars" + str(message.from_user.id) + ".jpg"
    dbutils.image_index(message.from_user.id, "avatars")
...
async def process_verify(message: types.Message, state: FSMContext):
    ...
    path = "../web/proj/media/verify/" + str(message.from_user.id) + ".jpg"
    dbutils.image_index(message.from_user.id, "verify")
    await message.photo[-1].download(path)
    await message.answer("Фото сохранено")
    await state.finish()
...
def register_handlers_avatar(dp: Dispatcher):
    dp.register_callback_query_handler(
        cmd_avatar, lambda call: call.data == "menu_avatar")
    dp.register_message_handler(yes_avatar, Text(
        equals="Да"), state=AvatarStates.confirm)
    dp.register_message_handler(no_avatar, Text(
        equals="Нет"), state=AvatarStates.confirm)
    dp.register_message_handler(process_avatar, content_types=[
                                'photo'], state=AvatarStates.send)


def register_handlers_verify(dp: Dispatcher):
    dp.register_callback_query_handler(
        cmd_verify, lambda call: call.data == "menu_verify")
    dp.register_message_handler(yes_verify, Text(
        equals="Да"), state=VerifyStates.confirm)
    dp.register_message_handler(no_verify, Text(
        equals="Нет"), state=VerifyStates.confirm)
    dp.register_message_handler(process_verify, content_types=[
                                'photo'], state=VerifyStates.send)
