...
async def cmd_delete(call: types.CallbackQuery):
    await DeleteStates.confirm.set()
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
    buttons = ["Да", "Нет"]
    keyboard.add(*buttons)
    await call.bot.send_message(call.from_user.id, "Вы уверены, что хотите удалить учетную запись?", reply_markup=keyboard)
